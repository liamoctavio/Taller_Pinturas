...
public final class JwtAuthService {
    ...
    // Hemos eliminado las variables estáticas que leían el entorno para que no falle al inicio.

    private JwtAuthService() {}

    // Este método mantiene la misma firma (nombre y parámetros) para que el resto del código no se rompa.
    public static JWTClaimsSet validate(String authHeader) throws Exception {
        
        // LOGICA DE BYPASS (Seguridad desactivada)
        System.out.println("⚠️ ALERTA: Seguridad desactivada. Aceptando petición sin validar token.");

        // Creamos una "identidad falsa" para engañar al sistema
        // Así el backend cree que hay un usuario llamado "Usuario Local" conectado.
        JWTClaimsSet fakeClaims = new JWTClaimsSet.Builder()
                .subject("usuario-bypass-id")
                .issuer("https://fake-issuer.com")
                .audience("fake-audience")
                .claim("name", "Usuario Local (Bypass)")
                .claim("scp", "access_as_user") // scopes falsos
                .claim("roles", "admin")        // rol falso
                .expirationTime(new Date(System.currentTimeMillis() + 3600 * 1000)) // Expira en 1 hora
                .build();
